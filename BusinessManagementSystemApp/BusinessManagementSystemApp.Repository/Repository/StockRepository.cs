...
namespace BusinessManagementSystemApp.Repository.Repository
{
    public class StockRepository
    {
        ...
        public List<PurchaseDetails> ExpiredQuantity(Product product, DateTime startDate, DateTime endDate)
        {
            var aProduct = db.PurchaseDetails.Include(c => c.Product).Where(c => c.Product.ProductName.ToLower() == product.ProductName.ToLower() && c.ExpireDate >= startDate && c.ExpireDate < endDate).ToList();
