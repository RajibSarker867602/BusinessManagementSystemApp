...
namespace BusinessManagementSystemApp.Repository.Repository
{
    public class ProductRepository
    {
        ...
        {
            Product aProduct = db.Products.FirstOrDefault(c => c.ProductId == ProductId);

            return aProduct;
        }
        public List<Product> GetProducts(Product product)
        {
            var products = db.Products.Include(c => c.Category).Where(c => c.Category.CategoryName.ToLower().Contains(product.CategoryName.ToLower()) || c.CategoryName.ToLower().Contains(product.CategoryName.ToLower())).ToList();
            return products;
        }
